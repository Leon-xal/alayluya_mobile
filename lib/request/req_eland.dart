...
class ReqEland {
  ...
  Future<Response> cate() {
    return _dio.post(G.prdapi + '/eland-cate');
  }
  ...
  /// 获取祈祷详情
  Future<Response> prayers_detail({int? id, int userid = 0}) {
  ...
  /// 获取文章的用户列表
  Future<Response> prayer_by_user({int? id, int userid = 0}) {
  ...
  /// 獲取eland詳情
  Future<Response> detail({int? eland_id, int userid = 0}) {
    print('userid======>${userid}');
    print('elandid======>${eland_id}');
    return _dio.post(
      G.prdapi + '/eland-detail',
      //        '/eland-detail',
      queryParameters: {'eland_id': eland_id, 'user_id': userid},
    );
  }

  /// 關注eland
  Future<Response> dofollow({int? eland_id, int? userid}) {
  ...
  /// 收藏eland動態內容
  Future<Response> docollect({int? id, int? userid, int? type}) {
    ...
    return _dio.post(
      G.prdapi + '/do-collect',
      //        '/do-collect',
      queryParameters: {'id': id, 'user_id': userid, 'type': type},
    );
  }

  /// 點擊祈祷
  Future<Response> doprayer({int? id, int? userid}) {
