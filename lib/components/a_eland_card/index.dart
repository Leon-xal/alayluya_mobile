...
class AElandCard extends StatefulWidget {
  final int uid;
  AElandCard({Key? key, this.uid = 0}) : super(key: key);
...
class _ElandCardState extends State<AElandCard>
  ...
  void initState() {
    ...
    Future.delayed(Duration.zero, () {
      _elandScrollController.addListener(() {
        if (_elandScrollController.position.pixels ==
            _elandScrollController.position.maxScrollExtent) {
          //          print('拉到底222===>');
          //          _loadElandListData( pageid: ++eland_page_id );
        }
      });
  ...
  _loadElandListData({int pageid = 1, int limit = 15}) async {
    try {
      ...
      ElandListModel eland_list = ElandListModel(
        code: 0,
        list: [],
        msg: '',
      ); // Default value
      ...
      if (res.data != null) {
        ...
        if (result['list'] is List) {
          (result['list'] as List).add({
            'eland_name': 'last_plus_button123456',
          });
          eland_list = ElandListModel.fromJson(result);
          // ... use eland_list ...
        } else {
          //Handle the case where result['list'] is not a list
          print('Error: result["list"] is not a list');
        }
      } else {
        //Handle the case where res.data is null
        print('Error: res.data is null');
      }
            ...
            child: new Column(
              children: <Widget>[
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    border: new Border(
                      bottom: BorderSide(width: 10.0, color: Color(0xFFCACBD1)),
                    ),
                    //              color: hex('#cacbd1'),
                    //                          shape: BoxShape.circle,
                  ),
