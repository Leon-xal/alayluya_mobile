...
//      print('aaa===>${widget.isShowcenterload}');
      _loadListData(context,isshowloading:widget.isShowCenterload,limit:widget.pagelimit,pageid:page_id,uid:widget.uid,search_by_name:widget.search_by_name,cateid:widget.cateid);
    });
  }
...
//            print('blockclick====>${item.eland_id}');
                        ...
                        Row(
                          ...
                          children: <Widget>[
                            Row(
                                children: <Widget>[
                                  Padding(
                                    ...
                                    child: Text(item.follow.toString()+'用戶',style: TextStyle(
                                      color: hex('#333'),
                                      fontSize: 13,
                                    )),
                                  ),
                                  Text('關注',style: TextStyle(
                                    color: hex('#333'),
                                    fontSize: 13,
                                  )),
                                ]
                            ...
                            Container(
                              ...
                              child:
                              (item.ifollow == true && item.follow > 0)?
                              AButton.icon(
  ...
  void _onRefresh() async{
    ...
    Future.delayed(Duration.zero,() async {
      if(mounted) {
        page_id = 1;
        dataItems = [];
        await _loadListData(context,isshowloading:false,limit:widget.pagelimit,pageid:page_id,cateid:widget.cateid,search_by_name:widget.search_by_name,uid:widget.uid,);
      }
  ...
  _loadListData(BuildContext context, {bool isshowloading=true, int pageid=1, int limit=5,uid=0,search_by_name='',cateid=0}) async {
...
//    super.build(context);
    ...
    if(widget.isSmartRefresher == true){
      if(isloadcomplete == true){
        return (dataItems.length == 0)?Container(
          child: Center(
...
//        header:G.pullToRefresh.header(),
      ...
      return (dataItems.length == 0) ? Container() : SingleChildScrollView(
          child: new Container(
            child: new Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: <Widget>[
