...
class _LoginStartState extends State<LoginStart> {
  ...
  void initState() {
    ...
    Future.delayed(Duration.zero, () async{
      ...
      setState(() {
        if (Platform.isIOS) {
          is_open_apple_login = (systemInfo == null)?false:systemInfo['is_open_apple_login'];
        }
        is_open_facebook_login = (systemInfo == null)?false:systemInfo['is_open_facebook_login'];
  ...
  Future<void> _loginByApple() async {
    ...
    final credential = await SignInWithApple.getAppleIDCredential(
      ...
      print('Leo sign in with apple id err========>>>>>>>>${err.toString()}');
    ...
    print('credential======>');
    if(credential == null){
      await G.toast('Login cancelled by the user.');
    }else{
  ...
  Future<void> _loginByFacebook() async {
    ...
    Provider.of<FacebookProvider>(context,listen: false).loginByFacebook().then((value) async {
      ...
      if(value == true){
        ...
        await Future.delayed(Duration.zero, () async {
          await G.toast(Provider.of<FacebookProvider>(context,listen: false).message);
          await Future.delayed(Duration(seconds: 3), () async {
            _loginByFacebook_i = false;
...
//                    color: rgba(255, 255, 255, 1),
                ...
                (is_open_apple_login == true) ? Container(
                  margin: EdgeInsets.only(bottom: 20),
                  child: Container(
                    width: 250,
                ...
                (is_open_facebook_login == true) ? Container(
                  margin: EdgeInsets.only(bottom: 20),
                  child: AButton.normal(
                     width: 250,
