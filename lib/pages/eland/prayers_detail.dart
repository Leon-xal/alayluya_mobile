...
class _PrayersDetailState extends State<PrayersDetail> {
  ...
  _loadData({int id = 0, int uid = 0}) async {
    try {
      var res = await G.req.eland.prayers_detail(id: id, userid: uid);
                    ...
                    children: <Widget>[
                      ...
                      new Column(
                        children: prayerByUserList.asMap().keys.map((f) {
                          ...
                          return Container(
                            margin: ((prayerByUserList.length - 1) == f)
                                ? const EdgeInsets.only(bottom: 0)
                                : const EdgeInsets.only(bottom: 15.5),
                            padding: ((prayerByUserList.length - 1) == f)
                                ? const EdgeInsets.only(bottom: 0)
                                : const EdgeInsets.only(bottom: 15.5),
                            decoration: ((prayerByUserList.length - 1) == f)
                                ? BoxDecoration()
                                : BoxDecoration(
                                    border: new Border(
                                      bottom: BorderSide(
